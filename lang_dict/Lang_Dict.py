...
class Lang_Dict:
    ...
    def __init__(self, lang_chosen):
        ...
        for row in cur.execute('select language, page_field, content from glossary_content;'):
            print(("select page_field from glossary_field where page_field=" + "'" + str(row[1]) + "';"))
            # Get code translation for the one obtained in cursor above. Translates for the page_field field
            page_field = cur_tmp.execute(("select page_field from glossary_field where field_id=" + "'" + str(row[1]) + "';"))
            page_field = page_field.fetchone()[0]
            dictionary[str(page_field)] = row[2]
